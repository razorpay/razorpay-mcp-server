...
// processVPAParameters handles VPA parameter processing
func processVPAParameters(params map[string]interface{}) {
	if vpa, exists := params["vpa"]; exists && vpa != "" {
		// Set method to UPI
		params["method"] = "upi"
		// Set UPI parameters with defaults
...
func InitiatePayment(
	...
	parameters := []mcpgo.ToolParameter{
		...
		mcpgo.WithString(
			"method",
			mcpgo.Description("Payment method (e.g., 'upi', 'card')"),
		),
		mcpgo.WithBoolean(
			"save",
	...
	handler := func(
		...
		// Process VPA parameters
		processVPAParameters(params)
	...
	return mcpgo.NewTool(
		...
		"Initiate a payment using the S2S JSON v1 flow. "+
			...
			"For UPI collect flow, simply provide 'vpa' parameter "+
			"which automatically sets UPI with flow='collect' and expiry_time='6'. "+
			"Supports additional parameters like method, customer_id, email, "+
			"contact, and save. "+
